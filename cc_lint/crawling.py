...
def get_warc_stream(path, use_s3=False, cache_dir=None):
    ...
    if os.path.exists(local_path):
        # Cache hit
        logging.getLogger(__name__).info(f"Cache hit: {local_path}")
        return open(local_path, 'rb')
    
    # Cache miss
    logging.getLogger(__name__).info(f"Cache miss: Downloading {path} to {local_path}")
    ...
    try:
        if use_s3:
            s3 = boto3.client('s3')
            s3.download_file('commoncrawl', path, temp_path, ExtraArgs={'RequestPayer': 'requester'})
        else:
            url = f"https://data.commoncrawl.org/{path}"
